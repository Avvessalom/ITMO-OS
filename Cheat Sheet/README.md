...
5. Как работают системные вызовы?
        ...
        Для взаимодествия с системными ресурсами приложения используют системные
        вызовы, которые дают возможность операционной системе обеспечить безопасный
        доступ к ним. Системные вызовы передают управление ядру операционной системы,
        которое определяет, предоставлять ли приложению запрашиваемые ресурсы или нет.
        Если ресурсы доступны, то ядро выполняет запрошенные действия, затем возвращает
        управление приложению.
        Происходит это, через прерывания. Прерывания передают управление ядру ОС,
        при этом приложению нужно внести в определенные регистры процессора,
        номер системного вызова и необходимые аргументы. Далее выполнить инструкцию
        генерации программного прерывания.
...
7. В какие системные вызовы раскрывается pthread_create?
        ...
        нууууууу или `clone system call` - клонируем родительский процесс
...
11. Другие флаги MMAP

        MAP_SHARED - сделать отображение общим. Изменения отображения видны всем процессам, отображающим ту же область
        MAP_PRIVATE - закрытое отображение с механизмом копирования при записи. Изменения отображения не видны другим процессам
        и т.д.
